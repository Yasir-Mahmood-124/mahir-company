...
const SofaCleaningServicesSection: React.FC<SofaCleaningServicesSectionProps> = ({ 
  ...
  const filteredServices = products.filter(
    (product) => product.subCategory === "Sofa Cleaning Services"
  );
  ...
  if (displayServices.length === 0) {
    return (
      <Box sx={{ py: 6, bgcolor: '#f5f5f5' }}>
        <Container maxWidth="lg">
          <Alert severity="info">No Sofa Cleaning Services available at the moment. Add services from the dashboard!</Alert>
        </Container>
      </Box>
    );
  }
