...
const MassageService: React.FC<MassageServiceProps> = ({ 
  title = "Massage Services",
  subtitle = "Professional Massage Services for Women",
  maxItems
}) => {
  ...
  const filteredServices = products.filter(
    (product) => product.subCategory === "Massage Services for Women"
  );
  ...
  if (displayServices.length === 0) {
    return (
      <Box sx={{ py: 6, bgcolor: '#f5f5f5' }}>
        <Container maxWidth="lg">
          <Alert severity="info">No Massage Services available at the moment. Add services from the dashboard!</Alert>
        </Container>
      </Box>
    );
  }
