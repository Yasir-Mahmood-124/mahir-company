...
const CurtainServicesSection: React.FC<CurtainServicesSectionProps> = ({ 
  ...
  const filteredServices = products.filter(
    (product) => product.subCategory === "Curtain Cleaning Services"
  );
  ...
  if (displayServices.length === 0) {
    return (
      <Box sx={{ py: 6, bgcolor: '#f5f5f5' }}>
        <Container maxWidth="lg">
          <Alert severity="info">No Curtain Cleaning Services available at the moment. Add services from the dashboard!</Alert>
        </Container>
      </Box>
    );
  }
...
export default CurtainServicesSection;