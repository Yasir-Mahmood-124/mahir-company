// ============================================
// WaterTankCleaningServiceCard.tsx
// ============================================
'use client';
...
const WaterTankCleaningServicesSection: React.FC<WaterTankCleaningServicesSectionProps> = ({ 
  ...
  const filteredServices = products.filter(
    (product) => product.subCategory === "Plastic Water Tank Cleaning Services"
  );
  ...
  if (displayServices.length === 0) {
    return (
      <Box sx={{ py: 6, bgcolor: '#f5f5f5' }}>
        <Container maxWidth="lg">
          <Alert severity="info">No Water Tank Cleaning Services available at the moment. Add services from the dashboard!</Alert>
        </Container>
      </Box>
    );
  }
